...
export async function createDeposit(fixture, overrides: any = {}) {
  ...
  if (longTokenAmount.gt(0)) {
    const longToken = await contractAt("MintableToken", overrides.longToken?.address || market.longToken);
    await longToken.mint(depositStore.address, longTokenAmount);
  }

  if (shortTokenAmount.gt(0)) {
    const shortToken = await contractAt("MintableToken", overrides.shortToken?.address || market.shortToken);
